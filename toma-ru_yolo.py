...
# ディレクトリの指定はここ
filename = '/home/maetaka10/time_log/'+now.strftime('%Y%m%d_%H') + '.csv'
...
class FrametimeCounter:
    ...
    def write_out(self):
        with open(f"frametime{self.file_count}.txt", "w") as f:
...
class Results(SimpleClass):
    ...
    def __init__(self, orig_img, path, names, boxes=None, masks=None, probs=None, keypoints=None) -> None:
        ...
        self.frametime_counter = FrametimeCounter(100)
    ...
    def save_txt(self, txt_file, save_conf=False):
        ...
        if texts:
            judgetexts = []
            list_num = len(texts)
            judgetexts = texts.copy()
            #texts = []
            #boxes = []
            detect_list = []
            for i in range(list_num):
                element = judgetexts[i]
                top_element = element[0:2]
                detect_list.append(top_element)
                detect_list = list(map(int,detect_list))
                print(detect_list)
                for i in filter(lambda j: j == 0 , detect_list):
                    if carnum == 0:
                        ...
                        print(texts)
                        print('carnum = ',carnum)
                        detect_opencv_yolov8.count_pulseTime()
                        carnum = detect_opencv_yolov8.servo(carnum)
                        carnum = 0
                        count_file2.kyori()
                        detect_opencv_yolov8.dorareko()
                        time.sleep(1)
...
#                            print('wheelnum = ',wheelnum)
#                            detect_opencv_yolov8.count_pulseTime()
#                            wheelnum = detect_opencv_yolov8.servo(wheelnum)
